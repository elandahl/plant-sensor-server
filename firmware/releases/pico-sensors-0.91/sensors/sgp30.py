...
def _ensure_init(i2c, addr):
    ...
    time.sleep(0.01)
    # Datasheet wants ~1 Hz Measure_air_quality; without that the
    # chip stays on fixed 400/0. Burn through the ~15 s init phase once.
    for _ in range(16):
        i2c.writeto(addr, CMD_MEASURE_AIR_QUALITY)
        time.sleep(1.0)
        try:
            _read_words(i2c, addr, 2)
        except OSError:
            pass
...
def read(i2c, addr):
    ...
    first = addr not in _initialized
    ...
    # Still fixed after init burst (rare) — caller can retry next loop.
    if eco2 == 400 and tvoc == 0 and first:
        raise OSError("SGP30 warm-up")

    # Namespaced so ENS160/161 co2_ppm / tvoc_ppb are not overwritten.
    return {
        "sgp30_eco2_ppm": eco2,
        "sgp30_tvoc_ppb": tvoc,
    }
