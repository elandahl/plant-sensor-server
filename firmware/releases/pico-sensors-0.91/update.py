...
def _version_key(version):
    """Numeric sort key so pico-sensors-0.10 > pico-sensors-0.9."""
    if not version:
        return ()
    # Strip common prefix; compare trailing dotted numbers.
    name = version
    ...
    for part in name.split("."):
        try:
            ...
            # Non-numeric suffix (e.g. 0.4-test): keep lexical tie-break.
            parts.append(part)
    return tuple(parts)
...
def update_available(manifest, current_version):
    ...
    if target == current_version:
        return False
    if _version_key(target) < _version_key(current_version):
        print("OTA skip downgrade", target, "<", current_version)
        return False
    return True
